...
func DesktopInit() {
	...
	moduleHandler.RegisterModule(module.ModuleInfo{
		Name:        "Desktop",
		Desc:        "Native AlpNAS desktop and operations workspace",
...
func desktop_handleUserInfo(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		...
		targetIsAdmin := searchingUser.IsAdmin()
		if !targetIsAdmin {
			for _, name := range searchingUser.GetUserPermissionGroupNames() {
				group := permissionHandler.GetPermissionGroupByName(name)
	...
	if !isAdmin {
		for _, name := range pgs {
			if permissionHandler != nil && permissionHandler.GetPermissionGroupByName(name) != nil && (permissionHandler.GetPermissionGroupByName(name).IsAdmin || permission.IsAdministratorGroupName(name)) {
				isAdmin = true
				break
			}
		}
