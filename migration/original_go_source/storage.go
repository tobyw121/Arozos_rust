...
func LoadBaseStoragePool() error {
	...
	appendHandler := func(handler *fs.FileSystemHandler) {
		if handler == nil {
			return
		}
		for _, existing := range fsHandlers {
			if existing != nil && existing.UUID == handler.UUID {
				return
			}
		}
		fsHandlers = append(fsHandlers, handler)
	}
	...
	appendHandler(tmpHandler)
	for _, handler := range loadAlpnasNativeBaseHandlers(localFileSystem) {
...
func loadAlpnasNativeBaseHandlers(localFileSystem string) []*fs.FileSystemHandler {
	...
	candidates := []nativeRoot{
		{Name: "AlpNAS Root", UUID: "alpnas_root", Path: "/tank/"},
		{Name: "AlpNAS Apps", UUID: "alpnas_apps", Path: "/tank/apps/"},
		{Name: "AlpNAS Data", UUID: "alpnas_data", Path: "/tank/data/"},
		{Name: "Media", UUID: "alpnas_media", Path: "/media/"},
		{Name: "Mounts", UUID: "alpnas_mnt", Path: "/mnt/"},
	}

	results := []*fs.FileSystemHandler{}
	for _, candidate := range candidates {
		cleanPath := filepath.ToSlash(filepath.Clean(candidate.Path)) + "/"
		if !fs.FileExists(cleanPath) {
			continue
		}

		handler, err := fs.NewFileSystemHandler(fs.FileSystemOption{
			Name:       candidate.Name,
			Uuid:       candidate.UUID,
			Path:       cleanPath,
			Hierarchy:  "public",
		...
		if err != nil {
			systemWideLogger.PrintAndLog("Storage", "Failed to initiate AlpNAS native storage root: "+cleanPath+" "+err.Error(), err)
			continue
		}

		results = append(results, handler)
		systemWideLogger.PrintAndLog("Storage", handler.Name+" mounted natively as "+handler.UUID+":/", nil)
	}
