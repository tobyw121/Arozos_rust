...
func user_handleUserEdit(w http.ResponseWriter, r *http.Request) {
	...
	if opr == "" {
		...
		allAdministratorGroupUsers := []string{}
		for _, group := range permissionHandler.PermissionGroups {
			if group == nil || !group.IsAdmin && !permission.IsAdministratorGroupName(group.Name) {
				continue
			}
			usersInGroup, groupErr := userHandler.GetUsersInPermissionGroup(group.Name)
			if groupErr != nil {
				continue
			}
			for _, groupUser := range usersInGroup {
				if groupUser == nil {
					continue
				}
				if !utils.StringInArrayIgnoreCase(allAdministratorGroupUsers, groupUser.Username) {
					allAdministratorGroupUsers = append(allAdministratorGroupUsers, groupUser.Username)
				}
			}
		}
		if len(allAdministratorGroupUsers) > 0 {
			//Skip checking if error
			leavingAllAdminGroups := true
			for _, groupName := range newGroupKeys {
				if permission.IsAdministratorGroupName(groupName) {
					leavingAllAdminGroups = false
					break
				}
			}
			if len(allAdministratorGroupUsers) == 1 && userinfo.IsAdmin() && leavingAllAdminGroups {
