...
func generateSessionKeyString() string {
	key := make([]byte, 32)
...
func AuthInit() {
	...
	if *session_key == "" {
		...
		if !sysdb.KeyExists("auth", "sessionkey") {
			newSessionKey := generateSessionKeyString()
			sysdb.Write("auth", "sessionkey", newSessionKey)
			systemWideLogger.PrintAndLog("Auth", "New authentication session key generated", nil)
		}
		skeyString := ""
		if err := sysdb.Read("auth", "sessionkey", &skeyString); err != nil || skeyString == "" {
			skeyString = generateSessionKeyString()
			sysdb.Write("auth", "sessionkey", skeyString)
			systemWideLogger.PrintAndLog("Auth", "Authentication session key regenerated after database read failure", err)
		} else {
...
func AuthSettingsInit() {
	...
	// AlpNAS internal auth_request endpoint for Nginx-proxied privileged tools.
	// Returns 204 only for a logged-in ArozOS administrator.
	http.HandleFunc("/alpnas/auth/admin", func(w http.ResponseWriter, r *http.Request) {
