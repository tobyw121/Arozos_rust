...
func (ldap *ldapHandler) HandleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if passwordCorrect {
		...
		if !ldap.ag.UserExists(username) {
			...
			if err := ldap.ag.LoginUserByRequest(w, r, username, rememberme); err != nil {
				utils.SendErrorResponse(w, "Unable to create login session")
				return
			}
...
func (ldap *ldapHandler) HandleSetPassword(w http.ResponseWriter, r *http.Request) {
	...
	if isValid {
		...
		if !ldap.ag.UserExists(username) {
			...
			ldap.ag.Logger.LogAuth(r, true)
			if err := ldap.ag.LoginUserByRequest(w, r, username, false); err != nil {
				utils.SendErrorResponse(w, "Unable to create login session")
				return
			}
			utils.SendOK(w)
