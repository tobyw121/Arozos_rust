...
func validateUsername(username string) error {
	if username == "" {
		return errors.New("Username not defined or empty.")
	}
	if strings.Contains(username, "/") || strings.Contains(username, "\\") {
		return errors.New("Username contains invalid path characters")
	}
	if strings.Contains(username, "..") {
		return errors.New("Username contains invalid path sequence")
	}
	return nil
}
...
func buildSessionOptions(r *http.Request, maxAge int) *sessions.Options {
...
func NewAuthenticationAgent(sessionName string, key []byte, sysdb *db.Database, allowReg bool, loginRedirectionHandler func(http.ResponseWriter, *http.Request)) *AuthAgent {
	...
	store.Options = buildSessionOptions(nil, 3600)
...
func (a *AuthAgent) HandleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if passwordCorrect {
		...
		if err := a.LoginUserByRequest(w, r, username, rememberme); err != nil {
			log.Println("[System Auth] Unable to persist login session for", username+":", err)
			sendErrorResponse(w, "Unable to create login session")
			return
		}
...
func (a *AuthAgent) ValidateUsernameAndPasswordWithReason(username string, password string) (bool, string) {
	...
	for _, candidate := range candidates {
		...
		err := a.Database.Read("auth", "passhash/"+candidate, &passwordInDB)
		if err != nil {
			continue
		}
...
func (a *AuthAgent) LoginUserByRequest(w http.ResponseWriter, r *http.Request, username string, rememberme bool) error {
	...
	if rememberme {
		session.Options = buildSessionOptions(r, 3600*24*7)
	} else {
		session.Options = buildSessionOptions(r, 3600*1)
	}
...
func (a *AuthAgent) HandleLogout(w http.ResponseWriter, r *http.Request) {
	...
	if fallbackAccount != "" {
		...
		if err := a.LoginUserByRequest(w, r, fallbackAccount, true); err != nil {
			log.Println("[System Auth] Unable to switch to fallback account:", err)
		}
	}
...
func (a *AuthAgent) Logout(w http.ResponseWriter, r *http.Request) error {
	...
	session.Options = buildSessionOptions(r, -1)
	session.Save(r, w)
...
func (a *AuthAgent) UserExists(username string) bool {
	...
	for _, candidate := range candidates {
		userpasswordhash := ""
		err := a.Database.Read("auth", "passhash/"+candidate, &userpasswordhash)
