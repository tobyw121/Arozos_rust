...
func (m *SwitchableAccountPoolManager) HandleAccountSwitch(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		...
		session.Options = buildSessionOptions(r, 3600*24*30)
		session.Save(r, w)
	}
	...
	if err != nil {
		...
		if err := m.authAgent.LoginUserByRequest(w, r, username, true); err != nil {
			utils.SendErrorResponse(w, "Unable to switch account session")
			return
		}
	} else {
