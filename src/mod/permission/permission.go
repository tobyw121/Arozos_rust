...
func IsAdministratorGroupName(name string) bool {
	switch normalizeGroupName(name) {
...
func isAdministratorGroupName(name string) bool {
	return IsAdministratorGroupName(name)
}
...
func (h *PermissionHandler) GroupExists(groupName string) bool {
	exists := false
	normalized := normalizeGroupName(groupName)
	for _, gp := range h.PermissionGroups {
		if normalized == normalizeGroupName(gp.Name) {
			exists = true
		}
	}
	return exists
...
func (h *PermissionHandler) GetUsersPermissionGroup(username string) ([]*PermissionGroup, error) {
	...
	if err != nil {
		...
		err = h.database.Read("auth", "group/"+normalized, &targetUserGroup)
		if err != nil {
			return []*PermissionGroup{}, err
		}
	}
	...
	results := []*PermissionGroup{}
	for _, gp := range h.PermissionGroups {
		if utils.StringInArrayIgnoreCase(permissionGroupNames, gp.Name) {
...
func (h *PermissionHandler) GetPermissionGroupByNameList(namelist []string) []*PermissionGroup {
	results := []*PermissionGroup{}
	for _, gp := range h.PermissionGroups {
		if utils.StringInArrayIgnoreCase(namelist, gp.Name) {
			thisPointer := gp
...
func (h *PermissionHandler) GetPermissionGroupByName(name string) *PermissionGroup {
	for _, gp := range h.PermissionGroups {
		if normalizeGroupName(name) == normalizeGroupName(gp.Name) {
			return gp
		}
	}
	return nil
